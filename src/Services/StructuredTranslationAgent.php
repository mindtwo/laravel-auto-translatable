<?php declare(strict_types=1);

namespace Mindtwo\AutoTranslatable\Services;

use Illuminate\Contracts\JsonSchema\JsonSchema;
use Illuminate\JsonSchema\Types\Type;
use Laravel\Ai\Contracts\Agent;
use Laravel\Ai\Contracts\HasStructuredOutput;
use Laravel\Ai\Promptable;
use Mindtwo\AutoTranslatable\Support\Config;

class StructuredTranslationAgent implements Agent, HasStructuredOutput
{
    use Promptable;

    /**
     * @param array<int, string> $fieldKeys
     */
    public function __construct(
        private string $systemPrompt,
        private array $fieldKeys,
    ) {}

    public function instructions(): string
    {
        return $this->systemPrompt;
    }

    public function provider(): string
    {
        return Config::string('auto-translatable.provider');
    }

    public function model(): string
    {
        return Config::string('auto-translatable.model');
    }

    public function maxTokens(): int
    {
        return Config::int('auto-translatable.output_tokens', 100000);
    }

    public function timeout(): int
    {
        return Config::int('auto-translatable.request_timeout', 500);
    }

    /**
     * Describe the expected structured output: one translated string per field key.
     *
     * @return array<string, Type>
     */
    public function schema(JsonSchema $schema): array
    {
        $properties = [];

        foreach ($this->fieldKeys as $key) {
            $properties[$key] = $schema->string()
                ->description("Translation of the \"{$key}\" field")
                ->required();
        }

        return $properties;
    }
}
